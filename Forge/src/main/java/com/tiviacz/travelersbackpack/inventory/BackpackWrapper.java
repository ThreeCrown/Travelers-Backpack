...
public class BackpackWrapper {
    ...
    public BackpackWrapper(ItemStack stack, int screenID, @Nullable Player player, @Nullable Level level, int[] dataLoad) {
        ...
        if(NbtHelper.getOrDefault(this.stack, ModDataHelper.UPGRADE_SLOTS, 0) != 0) {
            NbtHelper.set(this.stack, ModDataHelper.UPGRADE_SLOTS, 0);
        }
        if(NbtHelper.has(this.stack, ModDataHelper.STARTER_UPGRADES)) {
            NbtHelper.remove(this.stack, ModDataHelper.STARTER_UPGRADES);
        }
